...
class Ullmann
{
  ...
  public:
    ...
    tuple<bool, DFG*, int> Route_And_Reschedule(DFG* copy_DFG, int id, int number_of_resources, int II, int last_time)
    {
      ...
      int redo_attempt = -1; 
      bool redo = true;
      int sched_len, redoII;
      int org_size = copy_DFG_temp_in->getSetOfVertices().size(); 
      if(is_modified)
      {
        redoII = copy_DFG_temp_in->Schedule_1(id++, number_of_resources, II, sched_len, org_size);
        cout << "redoII: " <<redoII << endl;
      }
      ...
      return make_tuple(is_modified, copy_DFG_temp_in, redoII); 
    }
      ...
      while(current_II_2 <= MAPPING_POLICY.MAX_II)
      {
       ...
       // if(current_II_2 > current_II)
        ...
        while(map_attempt_per_II <= MAPPING_POLICY.MAPPING_ATTEMPTS_PER_II)
        {
          ...
          bool is_modified=false; 
          DFG* modified_DFG;
          tie(is_modified, modified_DFG, current_II_1) = Route_And_Reschedule(copy_DFG_2, id, current_number_resources, current_II_1, schedule_len);
          //int success = copy_DFG_2->Modulo_ReSchedule_Smart_2(id++, current_number_resources, current_II, nodes);
          cout << "is_modified: " << is_modified << endl;
