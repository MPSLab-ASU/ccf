...
Mapping_Info* Node::get_Mapping_Info()
{
  return map_info; 
}
    ...
    if((distance1 > this->get_Latency()) || (distance2 > this->get_Latency()) || (edge_distance > 0))
    {
      ...
      else	//iterate through the pair of vectors and insert in ascending order of distance
      {
       ...
       vector<int> dis; 
        ...
        //if(jj == dis.size())
       // {
       //   distance.insert(distance.begin()+jj+1,1,temp);
       //   nodes.insert(nodes.begin()+jj+1,1,outGoingArcs[i]->get_To_Node());
       // } 
      }
      retVal = true;
    }
...
bool MappingPair::MatchPE(MappingPair *mPair)
{
  if (mPair->getPE()->getIndexI() ==  pe->getIndexI() && mPair->getPE()->getIndexJ() == pe->getIndexJ()) 
  {
    return true;
  }
