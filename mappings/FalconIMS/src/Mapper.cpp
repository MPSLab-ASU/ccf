...
vector<vector<Cover_Pair*> > Mapper::MapWithoutRegisterFile(char *table, int tableSize, int cliqueSize, int II)
{
  ...
  /*ofstream adjgraph;
  cout << "printing the adjacency graph"<<endl;
  adjgraph.open("graph" + to_string(II) + ".txt");
  counter_II++; 
  adjgraph << tableSize << "\n";
...
vector<vector<Cover_Pair*> > Mapper::MapWithRegisterFile(char *table, int tableSize, int cliqueSize, int II)
{
  int CountNoImprove = 0;
  ...
  //Find NON Neighbors
  //From the adjacency graph if the any coordinate in the graph is 0. 
  vector<vector<int> > NonNeighbors;
  for (i = 0; i < (int) graph.size(); i++)
  {
    vector<int> NonNeighbor;
  ...
  ofstream cover_file;
  cover_file.open("covers.txt");
  cover_file<<covers.size();
  cover_file<<endl;
  for(int i=0; i<(int)covers.size(); i++)
  {
    if(i==0) { cover_file<<(int)covers[i].size(); cover_file<<endl; }
    for(int j=0; j<(int)covers[i].size(); j++)
      cover_file << " " << covers[i][j];
    cover_file << endl;
  }
  cover_file.close();
