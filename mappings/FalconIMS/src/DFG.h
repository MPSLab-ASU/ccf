...
class DFG
{
  ...
  public:
    ...
    // FUnctions to set the resources for scheduling
    void SetII(int II); 
    void SetResources(); 
    ...
    void Dot_Print_DFG_With_Schedule1(vector<Node*> nodes, string filename, int ID);
    ...
    int Schedule(int id, int number_of_resources);
    int Schedule_CRIMSON(int id, int number_of_resources);
    ...
    bool Modulo_Insert_Route(int &id, int number_of_resources, vector<Node*> &to_change_set, vector<Node*> &route);
    ...
    void GenerateAdjacencyForPath(int II); 

    int GetPath(int s, int d, int V);

    void GetPathUtil(int u, int d, bool visited[], int path[], int &path_index); 

    bool connected_Nodes2(Node* node1, Node* node2, int II); 

    void Prune_Routing(DFG* original, int II); 
    ...
    int set_height_based_priority();

    bool is_Executable(int height, int CGRASize);

    int MinTime(Node* current);
    ...
    bool Modulo_Schedule(bool &multiple_constraint, int number_of_resources);

    // modulo schedule modification
    bool Modulo_Schedule_2(bool &multiple_constraint, int number_of_resources);

    // Schedule routing operations
    bool Modulo_Schedule_Routing(int number_of_resources, vector<Node*> to_be_scheduled); 
