...
static Attr *handleLoopHintAttr(Sema &S, Stmt *St, const ParsedAttr &A,
  ...
  if(DEBUG) llvm::errs() << "handleLoopHintAttr hit\n"; //Arkan
  StringRef PragmaName =
      llvm::StringSwitch<StringRef>(PragmaNameLoc->Ident->getName())
          .Cases("unroll", "nounroll", "CGRA", "unroll_and_jam", "nounroll_and_jam", 
                 PragmaNameLoc->Ident->getName())
          .Default("clang loop");
  if(DEBUG) llvm::errs() << "PragmaName = " << PragmaName << "\n";
  ...
  if (St->getStmtClass() != Stmt::DoStmtClass &&
    ...
    if(DEBUG) llvm::errs() << "Pragma = " << Pragma << "\n";
  ...
  if (PragmaName == "nounroll") {
    SetHints(LoopHintAttr::Unroll, LoopHintAttr::Disable);
      ...
      if(DEBUG) llvm::errs() << "PragmaName == 'unroll'\n";
    // #pragma unroll N
      if (ValueExpr)
  ...
  } else if (PragmaName == "CGRA") {
      //Arkan
      if(DEBUG) llvm::errs() << "handleLoopHintAttr PragmaName == CGRA, SetHints\n";
      SetHints(LoopHintAttr::CGRA, LoopHintAttr::Enable);
      if(DEBUG) llvm::errs() << "handleLoopHintAttr SetHints completed\n";
      if(DEBUG) llvm::errs() << State << "\t" << Option << "\n";
      if(State == LoopHintAttr::Enable && Option == LoopHintAttr::CGRA)
          if(DEBUG) llvm::errs() << "State is Enabled and Option is CGRA\n";
  } else {
    // #pragma clang loop ...
      if(DEBUG) llvm::errs() << "handleLoopHintAttr else";
  ...
  if(DEBUG) llvm::errs() << "handlLoopHintAttr Reached end of function\n";

  if(Option == LoopHintAttr::CGRA) {
      if(DEBUG) llvm::errs() << "Option == LoopHintAttr::CGRA in handleLoopHintAttr\n"; 
  }
...
CheckForIncompatibleAttributes(Sema &S,
  ...
  if(DEBUG) llvm::errs() << "CheckForIncomptibleAttributes Hit\n";
  for (const auto *I : Attrs) {
    ...
    case LoopHintAttr::VectorizePredicate:
      ...
      break;
    case LoopHintAttr::CGRA:
      Category = CGRA;
      if(DEBUG) llvm::errs() << "CheckForIncomptibleAttributes Category is "<< Category << "\n";
      break;
    };
    ...
    if (Option == LoopHintAttr::Vectorize ||
        ...
        Option == LoopHintAttr::PipelineDisabled ||
        Option == LoopHintAttr::Distribute ||
        Option == LoopHintAttr::CGRA) {
      // Enable|Disable|AssumeSafety hint.  For example, vectorize(enable).
      if(DEBUG) llvm::errs() << "CheckForIncompatibleAttributes Option is "<< Option << "\n";
      PrevAttr = CategoryState.StateAttr;
      if(DEBUG) llvm::errs() << "CheckForIncompatibleAttributes PrevAttr from if statement is " << PrevAttr << "\n";
      CategoryState.StateAttr = LH;
      if(DEBUG) llvm::errs() << "CheckForIncompatibleAttributes StateAttr is " << LH << "\n";
      if(DEBUG) llvm::errs() << "CheckForIncompatibleAttributes CGRA StateAttr is" << CategoryState.StateAttr << "\n";
      if(DEBUG) llvm::errs() << "CheckForIncompatibleAttributes CGRA PrevAttr is" << CategoryState.StateAttr << "\n";
    } else {
      // Numeric hint.  For example, vectorize_width(8).
      if(DEBUG) llvm::errs() << "else statement PrevAttr is " << PrevAttr << "\n";
    ...
    if(DEBUG) llvm::errs() << "PrevAttr = " << PrevAttr << "\n";
    if (PrevAttr) {
      // Cannot specify same type of attribute twice.
      if(DEBUG) llvm::errs() << "CheckForIncompatibleAttributes if(PrevAttr)\n";
      if(PrevAttr->getDiagnosticName(Policy) == "")
          if(DEBUG) llvm::errs() << "getDiagnosticName is empty\n";
      if(DEBUG) llvm::errs() << "CheckForIncompatibleAttributes \t" << PrevAttr->getDiagnosticName(Policy)<< "\n";
      S.Diag(OptionLoc, diag::err_pragma_loop_compatibility)
          << /*Duplicate=*/true << PrevAttr->getDiagnosticName(Policy)
          << LH->getDiagnosticName(Policy);
      if(DEBUG) llvm::errs() << "CheckForIncomptibleAttributes if(PrevAttr) complete\n";
    }
    ...
    if (CategoryState.StateAttr && CategoryState.NumericAttr &&	
        (Category == CGRA &&
         CategoryState.StateAttr->getState() == LoopHintAttr::Enable)) {
      // CGRA Enable hints are not compatible with numeric hints of the same
      // category. added for #pragma CGRA shail
      if(DEBUG) llvm::errs() << "Category is CGRA and State is Enable. Not compatible with numeric hints of the same category\n";
      S.Diag(OptionLoc, diag::err_pragma_loop_compatibility)
          << false
          << CategoryState.StateAttr->getDiagnosticName(Policy)
          << CategoryState.NumericAttr->getDiagnosticName(Policy);
...
static Attr *ProcessStmtAttribute(Sema &S, Stmt *St, const ParsedAttr &A,
  ...
  case ParsedAttr::AT_LoopHint:
    if(DEBUG) llvm::errs() << "ProcessStmtAttribute hit\n";
...
StmtResult Sema::ProcessStmtAttributes(Stmt *S,
  ...
  for (const ParsedAttr &AL : AttrList) {
    if (Attr *a = ProcessStmtAttribute(*this, S, AL, Range)) {
      if(DEBUG) llvm::errs() << "if statement in Sema::ProcessStmtAttributes\n";
      Attrs.push_back(a);
    }
  }

  if(DEBUG) llvm::errs() << "Filled up Attr vector\n";
