...
struct PragmaCGRAHandler : public PragmaHandler { // for #pragma CGRA shail
  PragmaCGRAHandler(const char *name) : PragmaHandler(name) {}
  void HandlePragma(Preprocessor &PP, PragmaIntroducerKind Introducer,
                    Token &FirstToken) override;
};
...
void Parser::initializePragmaHandlers() {
  ...
  CGRAHandler.reset(new PragmaCGRAHandler("CGRA"));	//added for #pragma CGRA shail
...
void Parser::resetPragmaHandlers() {
  ...
  PP.RemovePragmaHandler(CGRAHandler.get()); // added for #pragma CGRA shail
  CGRAHandler.reset();
}
...
bool Parser::HandlePragmaLoopHint(LoopHint &Hint) {
  ...
  bool PragmaCGRA = PragmaNameInfo->getName() == "CGRA";
  if (Toks.empty() && (PragmaUnroll || PragmaNoUnroll || PragmaCGRA)) {	// added for #pragma CGRA shail
    ConsumeToken(); // The annotation token.
...
void PragmaCGRAHandler::HandlePragma(Preprocessor &PP,
                                     PragmaIntroducerKind Introducer,
                                     Token &Tok) {
  // Incoming token is "CGRA" for "#pragma CGRA"
  Token PragmaName = Tok;
  PP.Lex(Tok);
  auto *Info = new (PP.getPreprocessorAllocator()) PragmaLoopHintInfo;
  if (Tok.is(tok::eod)) {
    // CGRA pragma without an argument.
    Info->PragmaName = PragmaName;
    Info->Option.startToken();
  } else if (PragmaName.getIdentifierInfo()->getName() == "CGRA") {
    PP.Diag(Tok.getLocation(), diag::warn_pragma_extra_tokens_at_eol)
        << "CGRA";
    return;
  } 
